...
function bind(){const tap=(b,fn)=>b.onclick=()=>{b.classList.add('pressed');setTimeout(()=>b.classList.remove('pressed'),180);fn()};document.querySelectorAll('[data-k]').forEach(b=>tap(b,()=>key(b.dataset.k)));document.querySelectorAll('[data-s]').forEach(b=>tap(b,()=>sci(b.dataset.s)));document.querySelectorAll('[data-m]').forEach(b=>tap(b,()=>mem(b.dataset.m)));if(state.mode==='converter')bindConverter()}
