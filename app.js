...
function key(k){if(k==='AC'){state.expr='';state.result='0'}else if(k==='⌫'){state.expr=state.expr.slice(0,-1);state.result=state.expr||'0'}else if(k==='='){try{let e=sanitize(state.expr);e=e.replace(/(\d+(?:\.\d+)?)%/g,'($1/100)');let r=Function('return ('+e+')')();if(!Number.isFinite(r))throw Error();r=String(Math.round((r+Number.EPSILON)*1e12)/1e12);state.result=r;state.history.unshift({e:state.expr,r});state.history=state.history.slice(0,100)}catch{state.result='Error'}save();render();return}else if(k==='±'){state.expr=state.expr.startsWith('-')?state.expr.slice(1):'-'+state.expr;state.result=state.expr}else{const map={'×':'*','÷':'/','−':'-'};state.expr+=(map[k]??k);state.result=state.expr}save();render()}
