...
function key(k){if(k==='AC'){state.expr='';state.result='0'}else if(k==='⌫'){state.expr=state.expr.slice(0,-1);state.result=state.expr||'0'}else if(k==='='){calc();save();render();return}else if(k==='±'){state.expr=state.expr.startsWith('-')?state.expr.slice(1):'-'+state.expr;state.result=state.expr}else{const map={'×':'*','÷':'/','−':'-'};state.expr+=(map[k]??k);state.result=state.expr}save();render()}
