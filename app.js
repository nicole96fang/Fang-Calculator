...
function key(k){if(k==='AC'){state.expr='';state.result='0'}else if(k==='⌫')state.expr=state.expr.slice(0,-1);else if(k==='='){calc()}else if(k==='±'){state.expr=state.expr.startsWith('-')?state.expr.slice(1):'-'+state.expr}else{const map={'×':'*','÷':'/','−':'-'};state.expr+=(map[k]??k)}state.result=state.expr||'0';save();render()}
